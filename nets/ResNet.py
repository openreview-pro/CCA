...
#这里的reanet34是添加了跨通道图注意力机制的resnet34+CCA
'''
import torch
...
class SparseAttention(nn.Module):
    def __init__(self, dim, num_heads=1, k=7):
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.sparseattention = SparseAttention(dim=out_channels, num_heads=1, k=7)
...
class ResNet(nn.Module):
 ...
 -> (1, 1, 512) -> [Reshape] -> (512) -> [Linear] -> (10)
    ...
    def __init__(self, block, num_blocks, num_classes=100, verbose=False, init_weights=True):
